...
use crate::{
    ...
    util::{get_bool, get_owned_mutf8str, get_owned_optional_mutf8str},
};
...
pub struct CommandBlock<'a> {
    ...
    ///
    /// `LastOutput`
    pub last_output: Cow<'a, Mutf8Str>,
...
impl<'a> FromCompoundNbt for CommandBlock<'a> {
        ...
        Self: Sized,
    {
        ...
        let command = get_owned_mutf8str(&nbt, "command")?;
        let condition_met = get_bool(&nbt, "conditionMet");
        let custom_name = get_owned_optional_mutf8str(&nbt, "CustomName");
        let last_execution = nbt
            .long("LastExecution")
            .ok_or(SculkParseError::MissingField("LastExecution".into()))?;
        let last_output = get_owned_mutf8str(&nbt, "LastOutput")?;
