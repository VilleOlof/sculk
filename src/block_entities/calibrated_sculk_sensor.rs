...
fn test() {
    ...
    let sensor: CalibratedSculkSensor = fastnbt::from_value(&nbt).unwrap();

    assert_eq!(sensor.last_vibration_frequency, 10);
    assert_eq!(sensor.listener.event_delay, 5);
    assert_eq!(sensor.listener.selector.tick, 10);
    assert_eq!(sensor.listener.event.as_ref().unwrap().distance, 5.0);
    assert_eq!(
        sensor.listener.event.as_ref().unwrap().game_event,
        "minecraft:entity_born"
    );
    assert_eq!(sensor.listener.event.as_ref().unwrap().pos, [1.0, 2.0, 3.0]);
    assert_eq!(
        sensor.listener.event.as_ref().unwrap().projectile_owner,
        Some(79228162551157825753847955460)
    );
    assert_eq!(
        sensor.listener.event.as_ref().unwrap().source,
        Some(79228162551157825753847955460)
    );
    assert_eq!(sensor.listener.selector.event.distance, 5.0);
    assert_eq!(
        sensor.listener.selector.event.game_event,
        "minecraft:entity_born"
    );
    assert_eq!(sensor.listener.selector.event.pos, [1.0, 2.0, 3.0]);
    assert_eq!(
        sensor.listener.selector.event.projectile_owner,
        Some(79228162551157825753847955460)
    );
    assert_eq!(
        sensor.listener.selector.event.source,
        Some(79228162551157825753847955460)
    );

    let serialized_nbt = fastnbt::to_value(&sensor).unwrap();
